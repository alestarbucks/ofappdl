            ...
            while True:
                ...
                if " " in obj:
                    for obj_part in obj.split(" "):
                        if obj_part == " " or obj_part == "": continue
                        split_objects.append(obj_part + " - " + object_type)
                else: split_objects.append(obj + " - " + object_type)
        ...
        for pred in predicates:
            ...
            for arg in pred[1:]:
                if ' - ' in arg:
                    ...
                    has_type = False
                    for arg2 in pred[1:]:
                        if arg2 == arg: start = True
                        if start:
                            if ' - ' in arg2:
                                assert self.uses_typing
                                arg_type = self.types[arg2.strip().split(" - ", 1)[1].strip()]
                                arg_types.append(arg_type)
                                has_type = True
                                pred[index] += "- " + arg_type
                                break
        ...
        for match in matches:
            ...
            if self.uses_typing:
                for i in range(1, len(params)):
                    if len(params[i].strip().split("-", 1)) == 1:
                        j = i + 1
                        while len(params[j].strip().split("-", 1)) == 1:
                            j += 1
                        typing = params[j].strip().split("-", 1)[1].strip()
                        j -= 1
                        while j >= i:
                            params[j] = params[j] + " - " + typing
                            j -= 1
